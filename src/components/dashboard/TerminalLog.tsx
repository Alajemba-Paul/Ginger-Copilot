...
export default function TerminalLog({ logs }: TerminalLogProps) {
  ...
  return (
    <div className="w-full md:w-2/3 p-6 flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto bg-black p-4 rounded border border-gray-800 shadow-inner max-h-[50vh] md:max-h-full">
        {logs.map((log, index) => (
          <div 
            key={index} 
            className={`mb-2 text-sm ${log.includes('[REJECTED]') ? 'text-red-500' : 'text-[#00FF00]'}`}
          >
